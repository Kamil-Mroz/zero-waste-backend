...
public interface ItemRepository extends JpaRepository<Item, UUID> {
  @Query("SELECT i, o FROM Item i JOIN i.owner o JOIN i.category c WHERE o.id != ?1 and i.state != ?2")
  List<Item> findByOwner_IdNotAndStateNot(UUID ownerId, ItemState state);

  @Query("SELECT i, c FROM Item i JOIN i.owner o JOIN i.category c WHERE o.id = ?1")
  List<Item> findByOwner_Id(UUID ownerId);

  @Query("SELECT i, o FROM Item i JOIN i.owner o JOIN i.category c WHERE i.state != ?1")
