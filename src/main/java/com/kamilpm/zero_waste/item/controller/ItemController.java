package com.kamilpm.zero_waste.item.controller;
...
public class ItemController {
  ...
  public ResponseEntity<Void> publishItem(@PathVariable("id") UUID id) {
  ...
  public ResponseEntity<Void> hideItem(@PathVariable("id") UUID id) {
  ...
  @GetMapping
  public ResponseEntity<PageResponse<ItemListDto>> getItems(@RequestParam(value = "page", defaultValue = "0") int page,
      @RequestParam(value = "size", defaultValue = "20") int size,
      @RequestParam(value = "text", required = false) String text,
      @RequestParam(value = "category", required = false) UUID category) {

    Page<ItemListDto> items = itemService.getItems(PageRequest.of(page, size), text, category);
  ...
  @GetMapping("/own")
  public ResponseEntity<PageResponse<ItemListDto>> getOwnItems(
      @RequestParam(value = "page", defaultValue = "0") int page,
      @RequestParam(value = "size", defaultValue = "20") int size,
      @RequestParam(value = "text", required = false) String text,
      @RequestParam(value = "category", required = false) UUID category,
      @RequestParam(value = "states", required = false) List<ItemState> states) {
    Page<ItemListDto> itemsDto = itemService.getOwnItems(PageRequest.of(page, size), text, category, states);
  ...
  @GetMapping("/user/{id}")
  public ResponseEntity<List<ItemListDto>> getUsersItem(@PathVariable("id") UUID id) {
    List<ItemListDto> itemDtos = itemService.getUserItems(id);
